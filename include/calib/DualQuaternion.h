...
public:
  ...
  void fromScrew(T theta, T d, const Eigen::Matrix<T, 3, 1>& l, const Eigen::Matrix<T, 3, 1>& m);
...
template <typename T>
DualQuaternion<T>::DualQuaternion(const Eigen::Quaternion<T>& r, const Eigen::Quaternion<T>& d)
{
  m_real = r;
  m_dual = d;
}
...
template <typename T>
DualQuaternion<T>::DualQuaternion(const Eigen::Quaternion<T>& r, const Eigen::Matrix<T, 3, 1>& t)
{
  m_real = r.normalized();
...
template <typename T>
Eigen::Quaternion<T> DualQuaternion<T>::dual(void) const
{
  return m_dual;
}

template <typename T>
DualQuaternion<T> DualQuaternion<T>::exp(void) const
{
...
template <typename T>
void DualQuaternion<T>::fromScrew(T theta, T d, const Eigen::Matrix<T, 3, 1>& l, const Eigen::Matrix<T, 3, 1>& m)
{
...
template <typename T>
DualQuaternion<T> DualQuaternion<T>::identity(void)
{
  return DualQuaternion<T>(Eigen::Quaternion<T>::Identity(), Eigen::Quaternion<T>(0, 0, 0, 0));
}

template <typename T>
DualQuaternion<T> DualQuaternion<T>::inverse(void) const
{
...
template <typename T>
DualQuaternion<T> DualQuaternion<T>::log(void) const
{
...
template <typename T>
void DualQuaternion<T>::normalize(void)
{
...
template <typename T>
Eigen::Matrix<T, 3, 1> DualQuaternion<T>::transformPoint(const Eigen::Matrix<T, 3, 1>& point) const
{
  DualQuaternion<T> dq = (*this) *
...
template <typename T>
Eigen::Matrix<T, 3, 1> DualQuaternion<T>::transformVector(const Eigen::Matrix<T, 3, 1>& vector) const
{
  DualQuaternion<T> dq = (*this) *
                         DualQuaternion<T>(Eigen::Quaternion<T>(1, 0, 0, 0),
...
template <typename T>
Eigen::Quaternion<T> DualQuaternion<T>::real(void) const
{
  return m_real;
}
...
template <typename T>
DualQuaternion<T> DualQuaternion<T>::zeros(void)
{
  return DualQuaternion<T>(Eigen::Quaternion<T>(T(0), T(0), T(0), T(0)), Eigen::Quaternion<T>(T(0), T(0), T(0), T(0)));
}
...
DualQuaternion<T> DualQuaternion<T>::operator*(const DualQuaternion<T>& other) const
{
  return DualQuaternion<T>(m_real * other.m_real,
                           Eigen::Quaternion<T>((m_real * other.m_dual).coeffs() + (m_dual * other.m_real).coeffs()));
}
...
{
  out << dq.m_real.w() << " " << dq.m_real.x() << " " << dq.m_real.y() << " " << dq.m_real.z() << " " << dq.m_dual.w()
      << " " << dq.m_dual.x() << " " << dq.m_dual.y() << " " << dq.m_dual.z() << std::endl;
  return out;
}
