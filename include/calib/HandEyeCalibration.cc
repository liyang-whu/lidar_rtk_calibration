...
{
public:
  PoseError(Eigen::Vector3d r1, Eigen::Vector3d t1, Eigen::Vector3d r2, Eigen::Vector3d t2)
...
template <typename T>
static Eigen::MatrixXd ScrewToStransposeBlockofT(const Eigen::Matrix<T, 3, 1>& a, const Eigen::Matrix<T, 3, 1>& a_prime,
...
template <typename T>
static Eigen::MatrixXd AxisAngleToSTransposeBlockOfT(const Eigen::Matrix<T, 3, 1>& rvec1,
...
void HandEyeCalibration::estimateHandEyeScrew(
    ...
    const std::vector<Eigen::Vector3d, Eigen::aligned_allocator<Eigen::Vector3d>>& tvecs2, Eigen::Matrix4d& H_12,
    bool planarMotion)
{
...
void HandEyeCalibration::estimateHandEyeScrew(
    ...
    const std::vector<Eigen::Vector3d, Eigen::aligned_allocator<Eigen::Vector3d>>& rvecs2,
    const std::vector<Eigen::Vector3d, Eigen::aligned_allocator<Eigen::Vector3d>>& tvecs2, Eigen::Matrix4d& H_12,
    ceres::Solver::Summary& summary, bool planarMotion)
{
  size_t motionCount = rvecs1.size();
  Eigen::MatrixXd T(motionCount * 6, 8);
  T.setZero();
  ...
  Eigen::Vector3d t;
  t << 0, 0, 0;
  DualQuaterniond dq = DualQuaterniond(q, t);
...
// docs in header
DualQuaterniond HandEyeCalibration::estimateHandEyeScrewInitial(Eigen::MatrixXd& T, bool planarMotion)
{
...
// docs in header
bool HandEyeCalibration::solveQuadraticEquation(double a, double b, double c, double& x1, double& x2)
{
    ...
    ceres::Solver::Summary& summary)
{
  ...
  double p[7] = { dq.real().w(), dq.real().x(), dq.real().y(), dq.real().z(), H(0, 3), H(1, 3), H(2, 3) };
  ...
  for (size_t i = 0; i < rvecs1.size(); i++)
  {
    ...
    ceres::CostFunction* costFunction =
        new ceres::AutoDiffCostFunction<PoseError, 1, 4, 3>(new PoseError(rvecs1[i], tvecs1[i], rvecs2[i], tvecs2[i]));
